...
<template>
  <header class="leading-normal max-h-screen relative lg:flex lg:items-center lg:px-[calc(theme(spacing.section)/2)]">
    ...
    <div class="lg:flex lg:items-start lg:flex-wrap">
      ...
      <nav class="w-full text-xs text-center mt-8 lg:text-left lg:-ml-4 lg:text-base lg:py-4 lg:mt-4">
        ...
        <RouterLink to="/about" class="inline-block px-4 border-l border-surface-200 dark:border-surface-700">About</RouterLink>
      </nav>
    </div>

    <button class="absolute top-0 right-0 bg-transparent border-none cursor-pointer text-xl p-2 text-surface-900 dark:text-surface-0" @click="toggleDark" :aria-label="isDark ? 'Switch to light mode' : 'Switch to dark mode'">
